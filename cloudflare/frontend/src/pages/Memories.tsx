...
export function Memories() {
  ...
  return (
    <AppFrame
      left="memories"
      right={<span className="hl-mono" style={{ fontSize: 12, color: 'var(--bone-dim)', letterSpacing: '0.1em' }}>{memories.length} entries</span>}
    >
      {isLoading && (
      ...
      <div style={{
        ...
        {memories.map((m: any) => (
          ...
          >
            <div className="hl-mono" style={{ fontSize: 11, color: 'var(--bone-dim)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 8 }}>
              {new Date(m.createdAt ?? m.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
            </div>
            <p className="hl-serif" style={{ fontSize: 16, lineHeight: 1.7, color: 'var(--bone-dim)', margin: 0 }}>
