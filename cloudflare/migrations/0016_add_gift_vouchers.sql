...
-- ============================================
-- GIFT VOUCHER EMAILS LOG
-- ============================================

CREATE TABLE IF NOT EXISTS gift_voucher_emails (
  id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
  voucher_id TEXT NOT NULL REFERENCES gift_vouchers(id) ON DELETE CASCADE,
  email_type TEXT NOT NULL CHECK (email_type IN ('PURCHASE_CONFIRMATION', 'GIFT_SENT', 'GIFT_REMINDER', 'REDEMPTION_CONFIRMATION')),
  recipient_email TEXT NOT NULL,
  sent_at TEXT DEFAULT (datetime('now')),
  status TEXT DEFAULT 'SENT' CHECK (status IN ('SENT', 'FAILED', 'BOUNCED'))
);

CREATE INDEX idx_voucher_emails_voucher ON gift_voucher_emails(voucher_id);
