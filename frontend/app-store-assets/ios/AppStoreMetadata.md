# Heirloom - App Store Submission Requirements

## App Information

### Basic Info
- **App Name:** Heirloom
- **Subtitle:** Your Memories, Forever
- **Bundle ID:** blue.heirloom.blue
- **SKU:** HEIRLOOM001
- **Primary Language:** English (U.S.)
- **Category:** Lifestyle
- **Secondary Category:** Photo & Video

### Version Information
- **Version Number:** 1.0.0
- **Build Number:** 1
- **Copyright:** 2025 Heirloom

## App Store Listing

### Description (4000 characters max)
Heirloom is your digital sanctuary for preserving life's most precious moments. Create a lasting legacy through photos, voice recordings, and heartfelt letters that can be delivered to your loved ones at just the right moment.

**Key Features:**

Memories Vault
Store and organize your most treasured photos and videos with rich stories and context. Add emotions, locations, and tags to create a comprehensive archive of your life's journey.

Voice Recordings
Capture your voice with guided prompts that help you share wisdom, stories, and messages. Your authentic voice becomes a timeless gift for future generations.

Time-Capsule Letters
Write heartfelt letters to loved ones that can be delivered immediately, on a specific date, or posthumously. Each letter is sealed with care and delivered at the perfect moment.

Family Tree
Build your family tree and connect memories to the people who matter most. Share access with family members to create a collaborative legacy.

Dead Man's Switch
Set up automatic check-ins to ensure your legacy reaches your loved ones when the time comes. Designate trusted contacts who can verify and receive your precious content.

End-to-End Encryption
Your memories are protected with military-grade encryption. Only you and your designated recipients can access your content.

Year in Review (Wrapped)
Celebrate your journey with beautiful annual summaries of your memories, recordings, and letters.

**Subscription Plans:**
- Free Trial: 14 days with full access
- Keeper: Essential features for individuals
- Guardian: Advanced features for families
- Legacy: Premium features with priority support
- Eternal: Lifetime access with all features

Start preserving your legacy today. Your memories deserve to live forever.

### Keywords (100 characters max)
memories,legacy,family,photos,voice,letters,time-capsule,inheritance,vault,preservation,posthumous

### Promotional Text (170 characters max)
Preserve your most precious memories forever. Write letters, record your voice, and create a lasting legacy for your loved ones.

### What's New (4000 characters max)
Welcome to Heirloom 1.0!

We're thrilled to introduce Heirloom, your digital sanctuary for preserving life's most precious moments.

This initial release includes:
- Memories Vault for photos and videos
- Voice recording with guided prompts
- Time-capsule letters with scheduled delivery
- Family tree builder
- Dead Man's Switch for legacy delivery
- End-to-end encryption
- Year in Review (Wrapped)
- Beautiful dark theme with gold accents

Start your 14-day free trial today!

### Support URL
https://heirloom.blue/support

### Marketing URL
https://heirloom.blue

### Privacy Policy URL
https://heirloom.blue/privacy

## App Review Information

### Contact Information
- **First Name:** Reshigan
- **Last Name:** [Last Name]
- **Phone:** [Phone Number]
- **Email:** reshigan@vantax.co.za

### Demo Account
- **Username:** demo@heirloom.blue
- **Password:** demo123456

### Notes for Review
Heirloom is a digital legacy preservation app. Users can:
1. Upload photos and videos with stories
2. Record voice messages with guided prompts
3. Write letters for scheduled or posthumous delivery
4. Build family trees
5. Set up Dead Man's Switch for automatic legacy delivery

The app requires an internet connection to sync content. All content is encrypted end-to-end.

For testing the Dead Man's Switch feature, please note this is a safety feature that requires multiple check-ins over time. The demo account has this feature pre-configured for review purposes.

## Age Rating

### Content Descriptions
- Cartoon or Fantasy Violence: None
- Realistic Violence: None
- Sexual Content or Nudity: None
- Profanity or Crude Humor: None
- Alcohol, Tobacco, or Drug Use: None
- Mature/Suggestive Themes: None
- Simulated Gambling: None
- Horror/Fear Themes: None
- Medical/Treatment Information: None
- Unrestricted Web Access: No

**Recommended Age Rating:** 4+

## Pricing

### Price Tier
Free (with In-App Purchases)

### In-App Purchases
1. **Keeper Plan (Monthly)** - $4.99/month
2. **Keeper Plan (Annual)** - $49.99/year
3. **Guardian Plan (Monthly)** - $9.99/month
4. **Guardian Plan (Annual)** - $99.99/year
5. **Legacy Plan (Monthly)** - $19.99/month
6. **Legacy Plan (Annual)** - $199.99/year
7. **Eternal Plan (Lifetime)** - $499.99 one-time

## Screenshots Required

### iPhone 6.7" (iPhone 15 Pro Max)
1. Dashboard with desk and candle
2. Memories gallery view
3. Letter composition page
4. Voice recording with prompts
5. Family tree view
6. Settings page

### iPhone 6.5" (iPhone 14 Plus)
Same as 6.7"

### iPhone 5.5" (iPhone 8 Plus)
Same as 6.7"

### iPad Pro 12.9" (6th gen)
Same as iPhone but landscape orientation

### iPad Pro 12.9" (2nd gen)
Same as above

## App Icon Requirements

### Sizes Required
- 1024x1024 (App Store)
- 180x180 (iPhone @3x)
- 120x120 (iPhone @2x)
- 167x167 (iPad Pro @2x)
- 152x152 (iPad @2x)
- 76x76 (iPad @1x)

### Icon Design
- Gold infinity symbol on dark background (#050505)
- No transparency
- No rounded corners (iOS adds them automatically)

## App Capabilities

### Required Capabilities
- Camera (for photo capture)
- Microphone (for voice recording)
- Photo Library (for importing photos)
- Push Notifications (for reminders and delivery notifications)
- Face ID / Touch ID (for secure access)

### Background Modes
- Background fetch (for sync)
- Remote notifications

## Localization

### Supported Languages (Initial Release)
- English (U.S.) - Primary
- English (UK)
- English (Australia)

### Future Languages (Planned)
- Spanish
- French
- German
- Portuguese
- Japanese
- Chinese (Simplified)
