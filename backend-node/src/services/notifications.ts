...
export class NotificationService {
  ...
  async sendCheckInReminder(email: string, nextCheckIn: Date) {
  ...
  async sendMissedCheckInAlert(email: string, missedCount: number) {
    const checkInUrl = `${process.env.FRONTEND_URL}/check-in`;
  ...
  async sendEscalationAlert(email: string) {
    try {
      await this.resend.emails.send({
        from: 'Constellation Vault <noreply@loom.vantax.co.za>',
        to: email,
  ...
  async notifyTrustedContacts(userId: string) {
  ...
  async notifyRecipients(vaultId: string) {
